...
# from model import CNN as Network
...
parser.add_argument('--arch', type=str, default='CR18lei822245', help='which architecture to use')
...
fh = logging.FileHandler('./model/lamDARTS_CR-822218lei.txt')
...
def main():
    ...
    # criterion_triplet = TripletLoss(margin=5.0)
    # criterion_triplet = criterion_triplet.cuda()
    criterion_cent = CenterLoss(num_classes=CLASSES, feat_dim=256, use_gpu=True)
    ...
    for epoch in range(args.epochs):
        ...
        if valid_acc > best_val_acc:
            ...
            utils.save(model, os.path.join(args.save, 'lamDARTS_CR18lei-822245_best_weights.pt'))
        ...
        data.to_csv('result/lamDARTS_CR18lei-822245.xls', mode='a', header=False, index=False)
        utils.save(model, os.path.join(args.save, 'lamDARTS_CR18lei-822245_weights.pt'))
...
def train(train_queue, model, criterion, criterion_cent, optimizer, optimizer_centloss):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        # triplet_loss = criterion_triplet(features, target)
